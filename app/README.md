...
- **频谱翻转**（随音频加扰默认开）：倒放只改时间顺序，幅度谱不变，音高和音色都还在，听得出是男声还是女声；
  50 ms 这种短块连内容都大体听得懂。翻转把 164 Hz–10 kHz 上下颠倒（核心库 `SpectrumMirror`，16384 点 STFT），
  低音和 10 kHz 以上原样保留。加密时先倒放再翻转，解密反过来；元数据写 `mirror=0/1`，片头二维码 flags 第 1 位，
  旧文件没有这个字段按未翻转处理。翻转过的片头那一秒不是纯静音，而是一段 −40 dBFS 的 1–8 kHz 扫频音，
  供浏览器对齐块栅格。桌面端全链路实测（192k AAC 往返）比不加扰只多损失约 0.6 dB。
