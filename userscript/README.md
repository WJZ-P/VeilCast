...
1. 首次安装：在油猴管理器中新建脚本，把 `userscript/veilcast.user.js` 的**完整内容**粘贴进去并保存。
   从旧版升级：在原脚本编辑页全文替换、保存，再刷新视频页；当前版本 **0.1.7**，不要同时启用两份副本。
...
扫描在播放头位于前 1.5 秒或开始扫描后 5 秒内结束，从中途续播时不会触发；**把进度条拖回片头**（或按播放）会再扫一次，
所以中途加入的观众倒回开头即可自动还原。设置窗口里可以关闭这一行为。
...
- `src/audio.js`：定位并下载音轨、静音原声、还原音轨的播放同步；倒放、找栅格、WAV 编码在 `viewer/veilcast.js`。
