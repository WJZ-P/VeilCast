...
1. 首次安装：在油猴管理器中新建脚本，把 `userscript/veilcast.user.js` 的**完整内容**粘贴进去并保存。
   从旧版升级：在原脚本编辑页全文替换、保存，再刷新视频页；当前版本 **0.1.11**，不要同时启用两份副本。
...
3. **在信号里找块栅格**：容器和编码器会让音频整体偏移几十毫秒，浏览器解分片 MP4 时也不会去掉 AAC 的 1024 个前导样本，
   不校正的话还原结果是纯噪声（实测 −2.7 dB）。倒放在每个块边界都留下一个跳变，脚本在 ±100 ms 内逐样本找跳变能量最大的位置，
   实测能把 1024 样本的偏移找得分毫不差。
4. 逐块倒回来，编码成 WAV，用一个隐藏的 `<audio>` 和视频同步播放：暂停、跳转、倍速、音量都跟随视频；
...
- `src/audio.js`：定位并下载音轨、还原音轨的播放同步；准备期间保留原声，替代音轨成功播放后才静音原声，解码或播放失败时交还原声（不是已还原的声音）。倒放、找栅格、WAV 编码在 `viewer/veilcast.js`。
...
`userscript/tests/audio.html` 检查音频：先跑 `cargo test -p veilcast-app --test audio_pipeline`，它生成 250 ms 倒放、
按 AAC 64k 转码过的 `platform.mp4`（元数据已抹）和 B 站式纯音频分片 `platform.m4s`。页面先直接对两者做找栅格和倒放
（与原声对比约 14.5 dB，分片文件的 1024 样本偏移必须被找到），再让脚本读片头、自动下载还原、跟随暂停和跳转、停用后交还声音。
